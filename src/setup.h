...
struct Setup
{
	struct Debug : public Section
	{
		...
		{
			DIntValue(const std::string &s, const std::string &k)
					: IntValue(s, k), WrapFreeze()
			{
			}
		...
		{
			MValue(const std::string &s, const std::string &k)
			: Value<DModule>(s, k), WrapFreeze()
			  {
			  }
			DModule string2data(const std::string &s)
			{
